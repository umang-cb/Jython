...
class CBASInferSchema(CBASInferBase):
    ...
    def verify_analytics_supports_infer_schema(self):
        ...
        self.log.info('Execute INFER schema on dataset that does not exisit')
        status, _, error, _, _ = self.cbas_util.execute_statement_on_cbas_util('set `import-private-functions` `True`;SELECT Value array_infer_schema(%s)' % self.cb_bucket_name)
        self.assertTrue(status == "fatal", msg='Infer schema must fail as dataset does not exist')
        self.assertTrue('Cannot find dataset %s in dataverse Default nor an alias with name %s!' % (
                         self.cb_bucket_name, self.cb_bucket_name) in error[0]['msg'], msg='Error message mismatch')
        ...
        self.log.info('Execute INFER schema without import-private-functions')
        status, _, error, _, _ = self.cbas_util.execute_statement_on_cbas_util('SELECT Value array_infer_schema(%s)' % self.cbas_dataset_name)
        self.assertTrue(status == "fatal", msg='Infer schema must fail as dataset does not exist')
        self.assertTrue('Compilation error: function Default.array_infer_schema@1 is not defined' in error[0]['msg'], msg='Error message mismatch')
        self.assertEqual(error[0]['code'], 24001, msg='Error code mismatch')
        ...
        status, _, error, _, _ = self.cbas_util.execute_statement_on_cbas_util('set `import-private-functions` `False`;SELECT Value array_infer_schema(%s)' % self.cbas_dataset_name)
    ...
    def verify_infer_schema_on_travel_sample_dataset(self):
        ...
        self.load_sample_buckets(servers=[self.master], bucketName=self.cb_bucket_name, total_items=self.travel_sample_docs_count)
        self.cbas_util.createConn(self.cb_bucket_name)
